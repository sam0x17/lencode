...
impl Scheme for Lencode {
    ...
    fn encode_varint<I: UnsignedInteger>(val: I, writer: &mut impl Write) -> Result<usize> {
        // Fast path for zero and values <= 127 (common case)
        if val == I::ZERO {
            writer.write(&[0])?;
            return Ok(1);
        }
        ...
        // Fast path for small values (1 byte, <= 127)
        if le_bytes[1..].iter().all(|&b| b == 0) && le_bytes[0] <= 127 {
            writer.write(&[le_bytes[0]])?;
            return Ok(1);
        }
...
fn test_encode_decode_lencode_u8_all() {
    for i in 0..=255 {
        let val: u8 = i;
        let mut buf = [0u8; 1];
        let n = u8::encode::<Lencode>(&val, &mut Cursor::new(&mut buf[..])).unwrap();
        assert_eq!(n, 1);
        let decoded = u8::decode::<Lencode>(&mut Cursor::new(&buf)).unwrap();
        assert_eq!(decoded, val);
    }
}
